...
ValueNotifier<int> fieldUiState = ValueNotifier<int>(0);

class BuilderUI extends StatefulWidget {
  const BuilderUI({
    super.key,
  });
  static BuilderUIState? of(BuildContext context) {
    return context.findAncestorStateOfType<BuilderUIState>();
  }

  State<BuilderUI> createState() => BuilderUIState();
}

class BuilderUIState extends State<BuilderUI> {
  @override
  Widget build(BuildContext context) {
    return ValueListenableBuilder(
        valueListenable: fieldUiState,
        builder: (context, state, child) {
          log("------REBUILDING THIS UI-------");
          ...
          return Padding(
            ...
            child: LayoutBuilder(builder: (context, constra) {
              return Padding(
                padding:
                    EdgeInsets.symmetric(horizontal: constra.maxWidth * 0.2),
                child: Form(
                  key: formKey,
                  child: SizedBox(
